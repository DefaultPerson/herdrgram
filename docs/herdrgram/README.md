...
- `CCGRAM_TOPIC_ON_DEMAND` — новая сессия больше не создаёт топик сама: в General прилетает одно сообщение «🆕 Новая сессия: <имя> / 📁 <cwd> / 🤖 <провайдер>» с кнопками [🧵 Открыть топик] [🙈 Скрыть]. «Открыть» создаёт топик обычным путём (та же случайная иконка, то же имя, та же привязка), потом догружает хвост сессии: сначала шапка «⏮ В сессии N сообщений, пропущено M — ниже последние K» (или «⏮ Загружена вся история: N сообщений», если всё влезло; у молчащей сессии шапки нет вовсе), затем K сообщений по порядку в формате `/history` (👤 у реплик пользователя, тулколлы свёрнуты). K задаётся `CCGRAM_TOPIC_BACKFILL_MESSAGES` (по умолчанию 10, `0` выключает догрузку). Живой поток продолжается ровно с конца транскрипта на момент открытия — ничего не дублируется и «Jump to live» не всплывает. Предложение одно на окно, переживает рестарт бота, при смерти окна превращается в «⛔ Сессия завершена: <имя>» и забывается. «Скрыть» удаляет сообщение и запоминает окно навсегда (привязать всё ещё можно через `/sessions` и пикер `/new`). Состояние — `~/.ccgram/topic_announcements.json`.
