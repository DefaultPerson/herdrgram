...
def _bind_env(window: MagicMock | None = None) -> Iterator[_BindMocks]:
    ...
    with (
        ...
        # Returns the display name rather than a MagicMock: the rename
        # path records the title it wrote, and a record is JSON.
        patch(
            f"{_MODULE}format_topic_name_for_mode",
            side_effect=lambda display, _mode: display,
        ),
    ):
