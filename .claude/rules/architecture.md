...
- `show_command.py` — `/show` — raises the topic's session in the multiplexer UI via `multiplexer.focus_window`, gated on `capabilities.supports_focus` (herdr only). `focus_bound_window` is shared with the `show` toolbar builtin so both surfaces answer identically. Backend-neutral.
